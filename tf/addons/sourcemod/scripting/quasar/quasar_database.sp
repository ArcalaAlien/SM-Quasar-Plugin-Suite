...
{
    char s_formattedQuery[(MAX_QUERY_LENGTH*2)+1],
         s_moduleName[64];
    ...
    for (int i; i < columnList.Length; i++) {
        ...
        if (!s_tableName[0]) {
            FormatEx(s_keyName,
                    ...
                    SQL_TABLE_NAME_KEY,
                    i);
            h_columnInfo.GetString(s_keyName, s_tableName, sizeof(s_tableName));
        }
        ...
        FormatEx(s_keyName,
                 ...
                 SQL_COLUMN_NAME_KEY,
                 i);
        h_columnInfo.GetString(s_keyName, s_columnName, sizeof(s_columnName));

        // Reusing this variable since we don't
        // need it any more.
...
void SQLTxn_OnColumnInfoFetched(Database db, any data, int numQueries, DBResultSet[] results, QSRDBTable[] queryData)
{
    ...
    for (int i; i < numQueries; i++) {
        ...
        while (results[i].FetchRow()) {
            ...
            FormatEx(s_temp,
                     ...
                     SQL_TABLE_NAME_KEY,
                     i_currentColumn);
            h_columnInfo.SetString(s_temp, s_tableName);
    ...
    QSR_LogMessage(MODULE_NAME,
                   "SUCCESSFULLY CREATED COLUMN LISTS. MOVING TO CREATE INFO TABLES");
    Internal_SendDBTransaction(
...
void SQLTxn_OnTableFetchedForDump(Database db, any data, int numQueries, DBResultSet[] results, DataPack[] tableInfo)
{
    ...
    for (int i; i < numQueries; i++) {
        ...
        h_sqlDumpFile.WriteString(s_temp, false);
        ...
        for (int j; j < h_columnList.Length; j++) {
            ...
            FormatEx(s_columnKey,
                    ...
                    SQL_COLUMN_NAME_KEY,
                    j);
            h_columnInfo.GetString(s_columnKey, 
                                   s_columnName, 
                                   sizeof(s_columnName));
        ...
        while (results[i].FetchRow()) {
            ...
            for (int j;  j < h_columnList.Length; j++) {
                ...
                FormatEx(s_columnKey,
                        ...
                        SQL_COLUMN_DATA_TYPE_KEY,
                        j);
                h_columnInfo.GetString(s_columnKey, s_dataType, sizeof(s_dataType));
...
void SQLTxn_OnTablesFilled(Database db, int prevTransactionId, int numQueries, Handle[] results, QSRDBTable[] queryData)
{
    ...
    if (!QSR_OpenTransaction(prevTransactionId)) {
        QSR_LogMessage(MODULE_NAME,
                       "ERROR: COULD NOT REOPEN TRANSACTION %d IN SQLTxn_OnTablesFilled!",
                       prevTransactionId);
        return;
    }
    int i_transactionId = prevTransactionId;
